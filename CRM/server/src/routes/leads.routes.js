...
function mapImportedRow(row) {
  return {
    ...
    status: String(normalizeField(row, ['status']) || 'New').trim(),
  };
}
...
function applyStatusTimeline(lead, nextStatus) {
  ...
  if (nextStatus === 'Converted' && !lead.statusTimeline.convertedAt) {
    lead.statusTimeline.convertedAt = new Date();
  }
}
...
router.post('/', validate(createLeadSchema), asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ lead: serializeLead(populated) });
}));
...
router.patch('/:id', validate(updateLeadSchema), asyncHandler(async (req, res) => {
  ...
  const editableFields = ['name', 'email', 'phone', 'country', 'campaign', 'status'];
  if (isAdmin(req.user)) {
  ...
  editableFields.forEach((field) => {
    if (req.validated.body[field] !== undefined) {
      lead[field] = req.validated.body[field];
    }
  });

  if (req.validated.body.status !== undefined) {
    applyStatusTimeline(lead, req.validated.body.status);
  }
  ...
  req.app.get('io').emit('lead:updated', { lead: serializeLead(populated) });
